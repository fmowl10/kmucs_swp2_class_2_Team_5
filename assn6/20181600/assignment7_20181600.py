...
from calcFunctions import isRoman
from keypad import CONSTANT_DICT, FUNCTION_DICT, numPadList, operatorList
...
class Calculator(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        button_groups = OrderedDict({
            'num': {'buttons': numPadList, 'layout': num_layout, 'columns': 3},
            'op': {'buttons': operatorList, 'layout': op_layout, 'columns': 2},
            'constants': {'buttons': CONSTANT_DICT, 'layout': const_layout, 'columns': 1},
            'functions': {'buttons': FUNCTION_DICT, 'layout': func_layout, 'columns': 1},
        })
    ...
    def button_clicked(self):
        ...
        if sender.text() == '=':
            ...
            if text in CONSTANT_DICT.keys():
                text = '%.2f' % CONSTANT_DICT[text]
            
            if self.display.text().find(self.error_message) != -1:
                self.display.setText('0')
            if text in FUNCTION_DICT.keys():
                try:
                    if not isRoman(self.display.text()):
                        temp = str(eval(self.display.text()))
                    else:
                        temp = self.display.text()
                    result = FUNCTION_DICT[text](temp)
            ...
            if self.display.text()[-1] in op and text in op:
                return
            self.display.setText(self.display.text() + text)
        if len(self.display.text()) > self.display.maxLength():
            self.warn_box.show()
